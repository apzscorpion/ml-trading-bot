"""
Nifty50 Index Prediction Bot
Uses LSTM models to predict Nifty50 index movements.
"""
import numpy as np
import pandas as pd
...
try:
    ...
    logging.warning("TensorFlow not available. Nifty bot will use fallback predictions.")
...
class NiftyBot(BaseBot):
    """Nifty50 index prediction bot using LSTM"""
    
    name = "nifty_bot"
    
    def __init__(self):
        super().__init__("nifty_bot")  # Use string literal instead of self.name
        self.model = None
        self.scaler_X = None
        self.scaler_y = None
        self.sequence_length = 60
        self.symbol = "^NSEI"  # Yahoo Finance symbol for Nifty50
        self.model_path = "models/nifty_lstm_model.keras"
        self.scaler_path = "models/nifty_lstm_scalers.pkl"
    ...
    async def predict(
        ...
        """Generate Nifty50 prediction"""
        
        try:
            # If candles are empty or not for Nifty, fetch fresh data
            if not candles or len(candles) < self.sequence_length:
                logger.info(f"{self.name} fetching fresh Nifty50 data")
